...
router
  .route("/profile/edit")
  .get(freelancerController.getEditFreelancerProfile);
...
router
  .route("/skills_badges/quiz/:skillId")
  .get(freelancerController.getSkillQuiz);
router
  .route("/skills_badges/quiz/:skillId")
  .post(freelancerController.submitSkillQuiz);

// API endpoints for AJAX functionality
router.get("/skills_badges/api/refresh", freelancerController.refreshSkillsAPI);
router.post(
  "/skills_badges/quiz/:skillId/start",
  freelancerController.startQuizAPI
);
router.get(
  "/skills_badges/api/progress-report",
  freelancerController.generateProgressReportAPI
);

router.route("/milestone/:jobId").get(freelancerController.getMilestone);
router.post(
  "/milestone/:jobId/:milestoneId/request",
  freelancerController.requestMilestone
);
...
router.post(
  "/subtask/:jobId/:milestoneId/:subTaskId/update",
  freelancerController.updateSubTaskStatus
);
router.post(
  "/active_job/complain/:jobId",
  freelancerController.submitComplaint
);
