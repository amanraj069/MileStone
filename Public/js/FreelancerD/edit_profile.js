document.addEventListener("DOMContentLoaded", () => {
  ...
  function initializeProfileData() {
    ...
    editableElements.forEach((element) => {
      ...
      if (key === "skills") {
        profileData[key] = Array.from(
          element.querySelectorAll(".skill-tag")
        ).map((tag) => tag.textContent);
      } else if (key === "experience" || key === "education") {
        profileData[key] = Array.from(
          element.querySelectorAll(".experience-item, .education-item")
        ).map((item) => ({
          title: item.querySelector("h5").textContent,
          date: item.querySelector(".experience-date, .education-date")
            .textContent,
        ...
        profileData[key] = Array.from(
          element.querySelectorAll(".portfolio-item")
        ).map((item) => ({
